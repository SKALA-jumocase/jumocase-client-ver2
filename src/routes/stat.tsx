...
function RouteComponent() {
  ...
  const processAgeGroupStats = (): AgeGroupStats[] => {
    const ageGroups = recommendations.reduce(
      (acc, rec) => {
        let ageGroup: string;
  ...
  const processGenderStats = (): GenderStats[] => {
    const genderGroups = recommendations.reduce(
      (acc, rec) => {
  ...
  const processDrinkCountStats = (): DrinkCountStats[] => {
    const drinkCountGroups = recommendations.reduce(
      (acc, rec) => {
  ...
  const processPopularLiquorStats = (): PopularLiquorStats[] => {
    const liquorGroups = recommendations.reduce(
      (acc, rec) => {
  ...
  return (
    <div className="mb-8 min-h-screen w-full py-8">
      <div className="mx-auto max-w-7xl px-4">
        <div className="mb-8 text-center">
          <h1 className="mb-2 text-3xl font-bold text-gray-900">전통주 추천 통계</h1>
          <p className="text-gray-600">사용자들의 전통주 선택 패턴을 분석해보세요</p>
          {error && (
            <div className="mt-4 rounded border border-yellow-400 bg-yellow-100 p-3 text-yellow-700">
              {error}
            </div>
          )}
        </div>
        ...
        <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-4">
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h3 className="mb-2 text-lg font-semibold text-gray-800">총 추천 수</h3>
            <p className="text-3xl font-bold text-blue-600">{recommendations.length}</p>
          </div>
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h3 className="mb-2 text-lg font-semibold text-gray-800">인기 전통주</h3>
            <p className="text-xl font-bold text-green-600">
              {popularLiquorStats[0]?.liquorName || "N/A"}
            </p>
          </div>
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h3 className="mb-2 text-lg font-semibold text-gray-800">주 연령대</h3>
            <p className="text-xl font-bold text-purple-600">
              {ageGroupStats.sort((a, b) => b.count - a.count)[0]?.age || "N/A"}
            </p>
          </div>
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h3 className="mb-2 text-lg font-semibold text-gray-800">성별 비율</h3>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          {/* 연령대별 통계 */}
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-4 text-xl font-semibold text-gray-800">연령대별 사용자</h2>
          ...
          {/* 성별 통계 */}
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-4 text-xl font-semibold text-gray-800">성별 분포</h2>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={genderStats}
          ...
          {/* 음주 빈도 통계 */}
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-4 text-xl font-semibold text-gray-800">음주 빈도별 분포</h2>
            <ResponsiveContainer width="100%" height={300}>
          ...
          {/* 인기 전통주 */}
          <div className="rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-4 text-xl font-semibold text-gray-800">인기 전통주 TOP 5</h2>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={popularLiquorStats}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="liquorName" />
                <YAxis />
                <Tooltip />
        ...
        <div className="mt-8 rounded-lg bg-white p-6 shadow-md">
          <h2 className="mb-4 text-xl font-semibold text-gray-800">최근 추천 기록</h2>
